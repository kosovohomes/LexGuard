...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <header className="sticky top-0 z-40 border-b border-border bg-background/85 backdrop-blur-md supports-[backdrop-filter]:bg-background/72">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 h-16 flex items-center gap-2">
          <button onClick={() => app.home()} className="flex items-center gap-2.5 mr-3 shrink-0 group" aria-label="LexGuard home">
            <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-primary text-primary-foreground shadow-xs transition-colors group-hover:bg-primary/90" aria-hidden>
              <Shield className="h-4 w-4" />
            </span>
            <span className={`lg-display text-[17px] leading-none ${app.discreet ? "hidden" : "hidden sm:inline"}`}>LexGuard</span>
          </button>
          ...
          <nav className="hidden md:flex items-center gap-0.5 flex-1" aria-label="Main">
            {navItems.map((n) => (
              <button
                ...
                className={`inline-flex h-9 items-center gap-1.5 rounded-md px-3 text-sm transition-colors ${
                  n.active
      ...
      <footer className="mt-auto border-t border-border bg-muted/40 pb-[env(safe-area-inset-bottom)]">
...
export function PageTitle({ title, sub, back }: { title: string; sub?: string; back?: () => void }) {
  ...
  return (
    <div className="mb-8">
      ...
      <h1 className="lg-display text-[1.75rem] sm:text-3xl leading-tight">{title}</h1>
...
export function StateBadge({ state }: { state: USState }) {
  return (
    <span className="inline-flex items-center rounded-md border border-primary/15 bg-primary/[0.07] text-primary px-2 py-0.5 text-xs font-semibold tracking-wide">
      {state}
    </span>
  );
}
